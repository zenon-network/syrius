...
void main() {
  ...
  group('CancelPlasmaBloc', () {
    ...
    setUp(() {
      ...
      when(() => plasmaApi.cancel(any())).thenReturn(template);
      when(
    ...
    test('initial state is correct', () {
      expect(bloc.state, const CancelPlasmaInitial());
    });

    blocTest<CancelPlasmaBloc, CancelPlasmaState>(
      'emits [loading, done] on success',
      build: () => bloc,
      act: (CancelPlasmaBloc bloc) => bloc.add(
        CancelPlasmaRequested(plasmaHash: emptyHash),
      ),
      verify: (_) {
        verify(() => zenonAddressUtils.refreshBalance()).called(1);
      },
      expect: () => <CancelPlasmaState>[
    ...
    blocTest<CancelPlasmaBloc, CancelPlasmaState>(
      ...
      setUp: () {
        when(() => plasmaApi.cancel(any())).thenThrow(FailureException());
      },
      build: () => bloc,
      act: (CancelPlasmaBloc bloc) => bloc.add(
        CancelPlasmaRequested(plasmaHash: emptyHash),
      ...
      expect: () => <Matcher>[
        isA<CancelPlasmaLoading>(),
        isA<CancelPlasmaFailure>().having(
          (CancelPlasmaFailure s) => s.exception,
          'exception',
